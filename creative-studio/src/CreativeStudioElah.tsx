    ...
    <aside className="flex h-full w-[306px] shrink-0 overflow-hidden border-r border-[#273044] bg-[#101620] text-slate-200">
      ...
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden">
        {activeTab === "elements" ? <ElementsPanel activateOnTap className="min-h-0 flex-1 overflow-y-auto" /> : <>
          <div className="flex h-12 items-center justify-between border-b border-[#273044] px-3"><h2 className="text-sm font-semibold text-white">{tabs.find((tab) => tab.id === activeTab)?.label}</h2><button type="button" onClick={() => fileInputRef.current?.click()} className="inline-flex items-center gap-1 rounded-md bg-cyan-400 px-2.5 py-1.5 text-[11px] font-semibold text-[#07111c] hover:bg-cyan-300"><Upload className="h-3 w-3" /> Upload</button><input ref={fileInputRef} type="file" multiple accept="video/*,audio/*,image/*" onChange={uploadFiles} className="sr-only" /></div>
